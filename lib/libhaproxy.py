...
class ProxyHelper:
    ...
    def configure_tls_ciphers(self, save=True):
        """Configure SSL and TLS ciphers to disable known weak encryption methods."""
        cipher_config = Config.Config((
            "ssl-default-bind-ciphers "
            "TLS13-AES-256-GCM-SHA384:TLS13-AES-128-GCM-SHA256:"
            "TLS13-CHACHA20-POLY1305-SHA256:EECDH+AESGCM:EECDH+CHACHA20"
        ))
        bind_options_config = Config.Config(
            "ssl-default-bind-options no-sslv3 no-tlsv10 no-tlsv11"
        )
        global_section = self.proxy_config.globall[0]
        for cfg in global_section.configs():
            if (
                cfg.keyword == "ssl-default-bind-ciphers"
                or cfg.keyword == "ssl-default-bind-options"
            ):
                global_section.remove_config(cfg)
            global_section.add_config(bind_options_config)
            global_section.add_config(cipher_config)
    ...
    def enable_stats(self, save=True):
        ...
        config_block.append(
            Config.Config("stats uri {}".format(self.charm_config["stats-url"]), "")
        )
